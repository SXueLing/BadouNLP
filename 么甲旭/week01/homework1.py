...
class TorchModel(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.linear = nn.Linear(input_size, num_classes)  # 线性层，将输入维度映射到类别数量
        self.activation = nn.Softmax(dim=1)  # Softmax 归一化函数，将输出转换为类别概率
        self.loss = nn.CrossEntropyLoss()  # 交叉熵损失函数，用于多分类任务
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            return self.loss(y_pred, y)  # 计算预测值和真实值的损失
        else:
            return y_pred  # 输出预测结果


# 生成一个样本，样本的生成方法，代表了我们要学习的规律
# 随机生成一个 5 维向量，向量中最大数字所在的维度作为类别
def build_sample():
    x = np.random.random(5)
    label = np.argmax(x)  # 获取最大值的索引作为类别
    return x, label
...
def evaluate(model):
    ...
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
        _, predicted = torch.max(y_pred.data, 1)  # 获取预测的类别
...
def predict(model_path, input_vec):
    ...
    _, predicted = torch.max(result.data, 1)  # 获取预测的类别
