...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, sentence1, sentence2=None, sentence3=None, margain=None):
        #同时传入两个句子
        ...
        if sentence3 is not None and sentence2 is not None:
            vector1 = self.sentence_encoder(sentence1)  # vec:(batch_size, hidden_size)
            vector2 = self.sentence_encoder(sentence2)
            vector3 = self.sentence_encoder(sentence3)
            triplet_loss = self.cosine_triplet_loss(vector1, vector2, vector3,margain)
            return triplet_loss
        else:
            return self.sentence_encoder(sentence1)
